...
class TestUpdateDB(unittest.TestCase):
	...
	def test_UpdateDB(self):
		...
		#Amiga 1994
		gameRows = Game(self.gdb).getFilteredGames(2, 0, 6, 0, likeStmnt)		
		self.assertTrue(gameRows != None)
		self.assertEqual(len(gameRows), 2)
		
		#Amiga Black Legend Ltd.
		gameRows = Game(self.gdb).getFilteredGames(2, 0, 0, 6, likeStmnt)
		self.assertTrue(gameRows != None)
		self.assertEqual(len(gameRows), 1)		
		
		#Amiga Sports, 1994, Black Legend Ltd.
		gameRows = Game(self.gdb).getFilteredGames(2, 4, 6, 6, likeStmnt)
		self.assertTrue(gameRows != None)
		self.assertEqual(len(gameRows), 1)
		
		#SNES 1st person shooter
		gameRows = Game(self.gdb).getFilteredGames(3, 8, 0, 0, likeStmnt)
		self.assertTrue(gameRows != None)		
		self.assertEqual(len(gameRows), 1)
		
		#SNES 1992
		gameRows = Game(self.gdb).getFilteredGames(3, 0, 7, 0, likeStmnt)
		self.assertTrue(gameRows != None)
		self.assertEqual(len(gameRows), 3)
		
		#SNES Nintendo
		gameRows = Game(self.gdb).getFilteredGames(3, 0, 0, 12, likeStmnt)
		self.assertTrue(gameRows != None)
		self.assertEqual(len(gameRows), 2)
		
		#SNES 1992 Racing Nintendo 
		gameRows = Game(self.gdb).getFilteredGames(3, 11, 7, 12, likeStmnt)
		self.assertTrue(gameRows != None)
		self.assertEqual(len(gameRows), 1)
