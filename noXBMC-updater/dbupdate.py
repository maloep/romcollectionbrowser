...
from pyscraper.pyscraper import *
from pyscraper.descriptionparserfactory import *
import util
from util import *
...
class DBUpdate:		
			...
			if(not romCollection.descFilePerGame and len(romCollection.scraperSites) > 0):
				...
				try:						
					...
					for result in parser.scanDescription(scraper.source, str(scraper.parseInstruction), scraper.encoding):
						...
						self.insertGameFromDesc(result, gamenameFromFile, romCollection, filenamelist, foldername, isUpdate, gameId, dialogDict)
				...
				for filename in files:
					...
					lastGameId = self.insertGameFromDesc(gamedescription, gamenameFromFile, romCollection, filenamelist, foldername, isUpdate, gameId, dialogDict)													
	...
	def insertGameFromDesc(self, gamedescription, gamename, romCollection, filenamelist, foldername, isUpdate, gameId, dialogDict=''):								
		...
		gameId = self.insertData(gamedescription, gamename, romCollection, filenamelist, foldername, isUpdate, gameId, dialogDict)
		
		return gameId
	
	
			
	def insertData(self, gamedescription, gamenameFromFile, romCollection, romFiles, foldername, isUpdate, gameId, dialogDict=''):
		...
		for path in romCollection.mediaPaths:
			...
			self.getThumbFromOnlineSource(gamedescription, path.fileType.name, fileName, dialogDict)
		...
		if(not artWorkFound):
			ignoreGamesWithoutArtwork = False
	...
	def getThumbFromOnlineSource(self, gamedescription, fileType, fileName, dialogDict=''):
