...
class Mobygames_Scraper(WebScraper):
	...
	def _get_search_params(self, **kwargs):
		...
		return {'title': '%s' %GameNameUtil().prepare_gamename_for_webrequest(kwargs['gamename']),
	...
	def retrieve_release(self, gameid, platformid):
		time.sleep(1.2)
		response = self.open_json_url(url=self._get_retrieve_release_url(gameid, platformid), params=self._get_retrieve_params())
	...
	def retrieve_covers(self, gameid, platformid):
		time.sleep(1.2)
		response = self.open_json_url(url=self._get_retrieve_covers_url(gameid, platformid), params=self._get_retrieve_params())
	...
	def retrieve_screenshots(self, gameid, platformid):
		time.sleep(1.2)
		response = self.open_json_url(url=self._get_retrieve_screenshots_url(gameid, platformid), params=self._get_retrieve_params())
